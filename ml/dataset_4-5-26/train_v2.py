"""
Training XGBoost Total People Counting (single-output) berbasis CSI WiFi.

Varian dari `train.py` (per-region) yang memprediksi `total_orang` (0..6).
Skrip ini menjalankan dua varian sekaligus untuk perbandingan langsung:
  - Klasifikasi : XGBClassifier (multi:softprob)
  - Regresi    : XGBRegressor (reg:squarederror, hasil di-round ke int)
...
Flow:
  ...
  6. K-fold CV pakai XGB classifier + regressor -> metrik per fold.
  7. Train model final (clf & reg) di seluruh data, simpan masing-masing joblib.

Run dari direktori `ml/`:
    python dataset_4-5-26/train_v2.py                # default: keduanya (clf + reg)
    python dataset_4-5-26/train_v2.py --task clf     # hanya klasifikasi
    python dataset_4-5-26/train_v2.py --task reg     # hanya regresi
    python dataset_4-5-26/train_v2.py --task both    # keduanya (eksplisit)
"""
...
def run_task(
    ...
    """task in {'clf', 'reg'}. Jalankan K-fold CV + final train + feature importance."""
    assert task in {"clf", "reg"}
    is_clf = task == "clf"
    factory = make_xgboost_pipeline if is_clf else make_xgboost_regression_pipeline
    ...
    # XGBClassifier butuh kelas 0..N-1 kontigu; total_orang punya gap (label 5 absen).
    # Encode global → fit/predict pakai label terkode, lalu decode kembali ke skala asli
    # supaya metric/report tetap dalam range ALL_LABELS [0..6].
    ...
    print(f"\n[Final-{task}] Train on full dataset ...")
    final_model = factory()
    t0 = time.time()
    final_model.fit(X, y_work)
    print(f"  fit done in {time.time() - t0:.1f}s")

    out_path = DATA_DIR / f"xgb_total_{task}_final.joblib"
    joblib.dump(
        {
            "model": final_model,
    ...
    csv_path = DATA_DIR / f"feature_importance_total_{task}.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train XGBoost untuk total people counting (clf, reg, atau keduanya)."
    )
    parser.add_argument(
        "--task",
...
def main() -> None:
    ...
    print("=" * 64)
    print("XGBoost Total People Counting (single-output)")
    ...
    for task in tasks:
        run_task(task, X, y_total, file_per_window)
