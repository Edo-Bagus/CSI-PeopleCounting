"""
K-Fold CV untuk RandomForest Total People Counting (single-output) berbasis CSI WiFi.
...
Flow:
  ...
  7. Simpan hasil CV + info best fold ke `cv_result_<task>.npz`.

Run dari direktori `ml/`:
    python dataset_4-5-26/train_v2_rf_cv.py                 # default: keduanya
    python dataset_4-5-26/train_v2_rf_cv.py --task clf      # hanya klasifikasi
    python dataset_4-5-26/train_v2_rf_cv.py --task reg      # hanya regresi
    python dataset_4-5-26/train_v2_rf_cv.py --task both     # keduanya (eksplisit)
"""
...
def run_cv(
    ...
    """task in {'clf', 'reg'}. Jalankan K-fold CV dan simpan hasil + info best fold."""
    ...
    best_fold_score: float = -np.inf if is_clf else np.inf
    best_fold_id: int = fold_ids[0]
    best_train_files: List[str] = []
    best_val_files: List[str] = []
    ...
    for k in fold_ids:
        ...
        if is_clf:
            ...
            accs.append(acc)
            fold_metric: float = acc
            is_better = fold_metric > best_fold_score
        else:
        ...
        if is_better:
            ...
            best_fold_id = k
            best_train_files = sorted(train_files)
            best_val_files = sorted(val_files)
    ...
    print(f"\n  best fold: {best_fold_id}  ({metric_name}={best_fold_score:.4f})")

    out_path = DATA_DIR / f"cv_result_{task}.joblib"
    joblib.dump(
        {
            "task": task,
            "fold_ids": fold_ids,
            "accs": accs if is_clf else None,
            "maes": maes if not is_clf else None,
            "rmses": rmses if not is_clf else None,
            "exacts": exacts if not is_clf else None,
            "best_fold_id": best_fold_id,
            "best_fold_score": best_fold_score,
            "best_fold_metric": metric_name,
            "best_train_files": best_train_files,
            "best_val_files": best_val_files,
    ...
    print(f"  saved CV result to {out_path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="K-Fold CV RandomForest untuk total people counting."
    )
    parser.add_argument(
        "--task",
...
def main() -> None:
    ...
    print("=" * 64)
    print("RandomForest K-Fold CV (single-output)")
    ...
    for task in tasks:
        run_cv(task, X, y_total, file_per_window)
